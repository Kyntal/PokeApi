...
let PokeDetailComponent = class PokeDetailComponent {
    constructor(pokemonService, activatedRouter) {
...
PokeDetailComponent.ctorParameters = () => [
...
let PokeTableComponent = class PokeTableComponent {
    constructor(pokeService, router) {
    ...
    getPokemon() {
        let pokemonData;
        for (let i = 1; i <= 150; i++) {
            this.pokeService.getPokemon(i).subscribe(res => {
                pokemonData = {
                    position: i,
...
PokeTableComponent.ctorParameters = () => [
    { type: _services_pokemon_service__WEBPACK_IMPORTED_MODULE_3__["PokemonService"] },
